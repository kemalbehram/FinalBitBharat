...
import com.mobiloitte.p2p.content.dao.PaymentMethodDao;
...
import com.mobiloitte.p2p.content.dto.PaymentMethodNewDto;
...
import com.mobiloitte.p2p.content.utils.APIUtils;
...
public class CreateAdvertisementServiceimpl extends AddConstants implements CreateAdvertisementService {
	...
	public Response<Object> createAdvertisement(P2PAdvertisementDto p2PAdvertisementDto, Long userId) {
		try {
			...
			p2pBuySellModel.setPriceValue(p2PAdvertisementDto.getPriceValue());
			p2pBuySellModel.setFixed(p2PAdvertisementDto.getFixed());
			p2pBuySellModel.setMargin(p2PAdvertisementDto.getMargin());
			p2pBuySellModel.setFloating(p2PAdvertisementDto.getFloating());
			...
			p2pBuySellModel.setTotalAmount(p2PAdvertisementDto.getTotalAmount());
			p2pBuySellModel.setFirstTimePrice(p2PAdvertisementDto.getPrice());
			...
			if (p2PAdvertisementDto.getMaxValue().compareTo(p2PAdvertisementDto.getMinValue()) < 0) {
				...
				Response<GetBalanceResponseDto> data1 = walletClient.getBalance(p2PAdvertisementDto.getCryptoCoin(),
						userId);
				Double data6 = Math.ceil(Double.valueOf(data1.getData().getWalletBalance().toString()));

				System.out.println(data6 * 10000000);
				if (((data6 * 10000000) > 0) && p2PAdvertisementDto.getOrderType().equals(OrderType.BUY)) {

					for (String fetched : p2PAdvertisementDto.getPaymentType()) {
						PaymentTypeMethod paymentTypeMethod = new PaymentTypeMethod();
						paymentTypeMethod.setP2pAdvertisement(p2pBuySellModel);
						paymentTypeMethod.setTypeName(fetched);

						paymentDao.save(paymentTypeMethod);
					}
				...
				} else if ((data1.getData().getWalletBalance().compareTo(p2PAdvertisementDto.getTotalAmount()) == 1)
						&& p2PAdvertisementDto.getOrderType().equals(OrderType.SELL)) {
					for (String fetched : p2PAdvertisementDto.getPaymentType()) {
						PaymentTypeMethod paymentTypeMethod = new PaymentTypeMethod();
						paymentTypeMethod.setP2pAdvertisement(p2pBuySellModel);
						paymentTypeMethod.setTypeName(fetched);
				...
				return new Response<>(205, "Not able to create p2p addvertisement due to low wallet balance");
			}
		}
	...
	public Response<Map<String, Object>> getBuySEllAdvertisementDetails(OrderType orderType, Long userId, Integer page,
		...
		if (orderType.equals(OrderType.BUY)) {
			...
			getBuyOrderList = createAdvertisementDao.findByOrderTypeAndIsDeletedAndExchangeStatusTypeAndFkUserIdNot(
					...
					PageRequest.of(page, pageSize, Direction.DESC, "priceValue"));
			...
			getBuyOrderList = createAdvertisementDao.findByOrderTypeAndIsDeletedAndExchangeStatusTypeAndFkUserIdNot(
					orderType, false, ExchangeStatusType.ENABLED, userId,
					PageRequest.of(page, pageSize, Direction.ASC, "priceValue"));
			getTotalCount = createAdvertisementDao.countByOrderTypeAndIsDeletedAndExchangeStatusTypeAndFkUserIdNot(
					orderType, false, ExchangeStatusType.ENABLED, userId);
		}
			...
			getBuyOrderList = createAdvertisementDao.findByIsDeletedAndExchangeStatusTypeAndFkUserIdNot(false,
					ExchangeStatusType.ENABLED, userId, PageRequest.of(page, pageSize, Direction.ASC, "priceValue"));
		...
		if (getBuyOrderList.hasContent()) {
			for (P2PAdvertisement method : getBuyOrderList.getContent()) {
				List<PaymentTypeMethod> list = paymentDao
						.findByp2pAdvertisementPeerToPeerExchangeId(method.getPeerToPeerExchangeId());
				if (!list.isEmpty()) {
					for (PaymentTypeMethod list2 : list) {
						map.put("p2pId", list2.getP2pAdvertisement().getPeerToPeerExchangeId());
						map.put("paymentType", list2.getTypeName());
		...
		try {
			if (peerToPeerExchangeId != null) {
				...
				if (!details.isPresent()) {
					...
					List<PaymentTypeMethod> detailsGet = paymentDao
							.findByp2pAdvertisementPeerToPeerExchangeId(details.get().getPeerToPeerExchangeId());
					...
					if (value.getData() == KycStatus.ACCEPTED) {
						...
						if (checkTest == p2PAdvertisementDto.getIsIdentifiedPeople()) {
							...
							p2pBuySellModel.setPaymentWindow(p2PAdvertisementDto.getPaymentWindow());
							p2pBuySellModel.setTotalAmount(p2PAdvertisementDto.getTotalAmount());
							p2pBuySellModel.setUpiId(p2PAdvertisementDto.getUpiId());
							p2pBuySellModel.setFkUserId(userId);
							p2pBuySellModel.setPriceValue(p2PAdvertisementDto.getPriceValue());
							p2pBuySellModel.setFixed(p2PAdvertisementDto.getFixed());
							p2pBuySellModel.setFloating(p2PAdvertisementDto.getFloating());
	...
	public Response<Object> getDetailsOfBuyByfkId(Long peerToPeerExchangeId) {
		try {
			...
			if (data.isPresent()) {
				Optional<P2PAdvertisement> data1 = createAdvertisementDao
						.findByPeerToPeerExchangeIdAndIsDeletedFalse(peerToPeerExchangeId);
				BigDecimal totalAmount = BigDecimal.ZERO;
				List<Trading> tradeData = tradingDao.findByPeerToPeerExchangeIdAndTradeStatusAndStatusType(
						peerToPeerExchangeId, TradeStatus.PENDING, StatusType.PENDING);
				for (Trading dataGet : tradeData) {
					totalAmount = totalAmount.add(dataGet.getTradeAmount());
				}
				BigDecimal subtractAmount = data.get().getPrice().subtract(totalAmount);
				Map<String, Object> map = new HashMap<>();
				map.put("remainingAmount", subtractAmount);
				map.put("maxValue", data.get().getMaxValue());
				map.put("minValue", data.get().getMinValue());
				map.put("price", data.get().getPrice());
				map.put("priceValue", data.get().getPriceValue());
				map.put("fixed", data.get().getFixed());
				map.put("floating", data.get().getFloating());
				...
				map.put("totalAmount", data.get().getTotalAmount());
	...
	public Response<Object> sendTradeRequest(Long userId, Long peerToPeerExchangeId, BigDecimal amountInRange,
		...
		Optional<P2PAdvertisement> data = createAdvertisementDao
				.findByPeerToPeerExchangeIdAndIsDeletedFalse(peerToPeerExchangeId);
		List<PaymentTypeMethod> list2 = paymentDao.findByp2pAdvertisementPeerToPeerExchangeId(peerToPeerExchangeId);
		BigDecimal totalAmount = BigDecimal.ZERO;
		List<Trading> tradeData = tradingDao.findByPeerToPeerExchangeIdAndTradeStatusAndStatusType(peerToPeerExchangeId,
				TradeStatus.PENDING, StatusType.PENDING);
		for (Trading dataGet : tradeData) {
			totalAmount = totalAmount.add(dataGet.getTradeAmount());
		}
		BigDecimal subtractAmount = data.get().getPrice().subtract(totalAmount);
		if (subtractAmount.compareTo(amountInRange) == -1) {
			return new Response<>(205, "trading is going on already");
		}
		if (!data.isPresent())
		...
		if (data.get().getPrice().compareTo(data.get().getMinValue()) == -1) {
			...
			return new Response<>(201, "price is less than minimum limit");
		}
		...
		if (value.getData() != null) {
			if (value.getData().equals(KycStatus.ACCEPTED)) {
				...
				if (data.get().getMaxValue().compareTo(amountInRange) >= 0
					...
					BigDecimal d3 = data.get().getPrice().divide(amountInRange, 40, RoundingMode.HALF_UP);
					...
					map1.put("coinName", data.get().getCryptoCoin());
					...
					map1.put("BankName", data.get().getBankName());
					map1.put("AccountNo", data.get().getAccountNo());
					map1.put("HolderName", data.get().getAccountHolderName());
					map1.put("upiId", data.get().getUpiId());
					map1.put("ifscCode", data.get().getIfscCode());
					map1.put("peerToPeerExchangeId", peerToPeerExchangeId);
					map1.put("paymentMethod", list2);
					map1.put("priceValue", data.get().getPriceValue());
					map1.put("fixed", data.get().getFixed());
					map1.put("floating", data.get().getFloating());
					map1.put("appliedValue", restrictAmount);
					list.add(map1);
					if (data.get().getOrderType() == OrderType.SELL) {
						...
						trading.setTotalAmount(data.get().getTotalAmount());
						trading.setPrice(data.get().getPrice());
						...
						trading.setAppliedValue(restrictAmount);
						map1.put("transactionStatus", TransactionStatus.PENDING);
						...
						if (value8.getData() != null) {
							...
							BigDecimal blockedBalance = data4.getBlockedBalance();
							Escrow escrow = new Escrow();
							if (walletBalance.compareTo(restrictAmount) > 0) {
								BigDecimal restWalletBalance = walletBalance.subtract(restrictAmount);
								walletClient.updateWallet(restWalletBalance, data.get().getFkUserId(),
										data.get().getCryptoCoin());
								BigDecimal restblockedBalance = blockedBalance.add(restrictAmount);
								walletClient.updateBlocked(restblockedBalance, data.get().getFkUserId(),
										data.get().getCryptoCoin());
								escrow.setWalletBalance(restWalletBalance);
								BigDecimal c = getChargeFromAdmin(data.get().getCryptoCoin());
								if (restrictAmount.compareTo(c) > 0) {
									...
									escrow.setAppliedValue(restrictAmount);
									...
									setData.put(EmailConstants.AMOUNT_TOKEN, String.valueOf(restrictAmount));
									setData.put(EmailConstants.PAYMENT_WINDOW_TOKEN,
											String.valueOf(data.get().getPaymentWindow()));
									setData.put(EmailConstants.COIN, data.get().getCryptoCoin());
						...
						if (value8.getData() != null) {
							...
							BigDecimal blockedBalance = data5.getBlockedBalance();
							Escrow escrow5 = new Escrow();
							if (walletBalance5.compareTo(restrictAmount) > 0) {
								BigDecimal restblockedBalance = blockedBalance.add(restrictAmount);
								BigDecimal restWalletBalance5 = walletBalance5.subtract(restrictAmount);
								walletClient.updateWallet(restWalletBalance5, userId, data.get().getCryptoCoin());
								walletClient.updateBlocked(restblockedBalance, userId, data.get().getCryptoCoin());
								...
								if (amountInRange.compareTo(c) > 0) {
									...
									escrow5.setAppliedValue(restrictAmount);
									trading.setPaybleAmount(amountInRange);
		...
		try {
			...
			if (data1.isPresent()) {
				...
				if (data.isPresent()) {
					...
					setData.put(EmailConstants.COIN, data.get().getCoinName());
					String jsonStr = GSON.toJson(map1);
					setData.put(EmailConstants.DATA_TOKEN, jsonStr);
	...
	public Response<Object> forReleaseBitcoins(String tradeId, Long userId) {
		try {
			...
			Response<GetBalanceResponseDto> value6 = walletClient.getBalance(data1.get().getCryptoCoin(), userId);
			BigDecimal BallanceOnWallet2 = value6.getData().getBlockedBalance();
			BigDecimal BallanceOnWallet3 = value5.getData().getWalletBalance();
			BigDecimal releaseAmount4 = BallanceOnWallet3.add(data1.get().getAppliedValue());
			BigDecimal releaseAmount3 = BallanceOnWallet2.subtract(data1.get().getAppliedValue());
			BigDecimal releaseAmountFinal = releaseAmount4.subtract(data.get().getTradeFees());
			BigDecimal releaseAmountFinal1 = releaseAmount3;

			if (data1.get().getTransactionStatus() == TransactionStatus.PAID
					&& data1.get().getTradeStatus() == TradeStatus.PAID) {
				walletClient.updateBlocked(releaseAmountFinal1, userId, data1.get().getCryptoCoin());
				walletClient.updateBlocked(releaseAmountFinal, data1.get().getPartnerId(), data1.get().getCryptoCoin());

				data.get().setBlockedBalance(BigDecimal.ZERO);
				data.get().setStatusType(StatusType.COMPLETE);
				data.get().setTransactionStatus(data1.get().getTransactionStatus());
				data1.get().setTradeStatus(TradeStatus.COMPLETE);
				data1.get().setIsDeleted(false);
				data3.get().setTotalAmount(data3.get().getTotalAmount().subtract(data1.get().getTotalBTC()));

				data3.get().setPrice(data3.get().getPrice().subtract(data1.get().getTotalPrice()));
				data3.get().setTotalAmount(data3.get().getTotalAmount().subtract(data1.get().getAppliedValue()));
				P2pHistoryDto p2pHistoryDto = new P2pHistoryDto();
				String apiBaseUri1 = "http://3.213.77.140:3015/eth/getMarketPriceById?ids="
						+ data1.get().getCryptoCoin().toLowerCase();
				...
				if (allData1.containsKey("USD")) {
					...
					p2pHistoryDto.setLiveAmount(value);
					p2pHistoryDto.setDepositLiveAmount(value.multiply(data1.get().getAppliedValue()));
				}

				p2pHistoryDto.setAmount(data1.get().getAppliedValue());
				p2pHistoryDto.setP2pId(data1.get().getPeerToPeerExchangeId());
				p2pHistoryDto.setTradeId(tradeId);
				p2pHistoryDto.setTradeStatus(data1.get().getTradeStatus().toString());
				p2pHistoryDto.setTransactionStatus(data1.get().getTransactionStatus().toString());
				p2pHistoryDto.setType(data1.get().getType().toString());
				p2pHistoryDto.setCoinName(data1.get().getCryptoCoin());
				walletClient.updateP2pData(p2pHistoryDto, userId);
				createAdvertisementDao.save(data3.get());
				escrowDao.save(data.get());
				tradingDao.save(data1.get());
				...
				if (data3.get().getOrderType() == OrderType.BUY) {
					...
					setData.put(EmailConstants.COIN, data3.get().getCryptoCoin());
					setData.put(EmailConstants.AMOUNT_TOKEN, String.valueOf(data1.get().getTotalBTC()));
					setData.put(EmailConstants.DATE_TOKEN, String.valueOf(new Date()));
					String jsonStr = GSON.toJson(map1);
					setData.put(EmailConstants.DATA_TOKEN, jsonStr);
	...
	@Override
	public Response<Object> forStatusChecking(Long peerToPeerExchangeId, String tradeId) {
		try {
			Optional<Trading> data = tradingDao.findByPeerToPeerExchangeIdAndTradeId(peerToPeerExchangeId, tradeId);
			Optional<P2PAdvertisement> data2 = createAdvertisementDao
					.findP2PAdvertisementByPeerToPeerExchangeId(peerToPeerExchangeId);
			Map<String, Object> map1 = new HashMap<>();
			if (data.isPresent() && data2.isPresent()) {
				DateTime createdTime = new DateTime(data.get().getCreationTime());
				Date time = createdTime.toDate();
				Integer timeDiff = Seconds.secondsBetween(createdTime, DateTime.now()).getSeconds();
	...
	@Override
	public Response<Map<String, Object>> getOpenTradeDetails(Long userId, Integer page, Long fromDate, String coin,
			OrderType orderType, Integer pageSize, Long toDate) {
		try {

			StringBuilder query = new StringBuilder(
					"select c.peerToPeerExchangeId, c.userName, c.exchangeStatusType, c.country, c.paymentType, c.orderType, c.creationTime,c.fiatCoin,c.maxValue,c.fkUserId,c.minValue,c.price,c.cryptoCoin,c.margin,c.paymentWindow,c.isIdentifiedPeople,c.twpfaType,c.termsOfTrade,c.totalAmount,c.priceValue,c.fixed,c.floating from P2PAdvertisement c");
			List<String> conditions = new ArrayList<>();
			if (userId != null) {
				conditions.add("((c.fkUserId like :userId))");
			}
			...
			if (coin != null) {
				conditions.add("((c.cryptoCoin like :coin))");
			}
			...
			if (coin != null) {
				createQuery.setParameter("coin", coin);
			}
			...
			List<SearchAdvertisementDto> response = list.parallelStream().map(o -> {
				...
				dto.setMinValue(ret);
				dto.setPrice(rot);
				dto.setCryptoCoin((String) o[12]);
				dto.setMargin((BigDecimal) o[13]);
				dto.setPaymentWindow((Integer) o[14]);
				dto.setIsIdentifiedPeople((Boolean) o[15]);
				dto.setTwpfaType((TwoFaType) o[16]);
				dto.setTermsOfTrade((String) o[17]);
				dto.setTotalAmount((BigDecimal) o[18]);
				dto.setPriceValue((BigDecimal) o[19]);
				dto.setFixed((String) o[20]);
				dto.setFloating((String) o[21]);
				return dto;
...
//				trading.setPaymentType(data.get().getPaymentType());
	...
	public Response<Map<String, Object>> getBuySEllAdvertisementList(OrderType orderType, Integer page,
			Integer pageSize, String cryptoCoin) {
		...
		if (orderType.equals(OrderType.BUY)) {
			...
			getBuyOrderList = createAdvertisementDao.findByOrderTypeAndIsDeletedFalseAndExchangeStatusType(orderType,
					ExchangeStatusType.ENABLED, PageRequest.of(page, pageSize, Direction.DESC, "priceValue"));
			...
			getBuyOrderList = createAdvertisementDao.findByOrderTypeAndIsDeletedFalseAndExchangeStatusType(orderType,
					ExchangeStatusType.ENABLED, PageRequest.of(page, pageSize, Direction.ASC, "priceValue"));
			getTotalCount = createAdvertisementDao.countByOrderTypeAndIsDeletedFalseAndExchangeStatusType(orderType,
					ExchangeStatusType.ENABLED);
	...
	public Response<Object> getAdvertisementSearch(Long userId, Long userId2, Long peerToPeerExchangeId,
			...
			String cryptoCoin, BigDecimal margin, Integer paymentWindow) {

		StringBuilder query = new StringBuilder(
				"select c.peerToPeerExchangeId, c.userName, c.exchangeStatusType, c.country, c.paymentType, c.orderType, c.creationTime,c.fiatCoin,c.maxValue,c.fkUserId,c.minValue,c.price,c.cryptoCoin,c.margin,c.paymentWindow,c.isIdentifiedPeople,c.twpfaType,c.termsOfTrade,c.totalAmount,c.priceValue,c.fixed,c.floating from P2PAdvertisement c where c.fkUserId not in ('userId')");
		List<String> conditions = new ArrayList<>();
		if (peerToPeerExchangeId != null) {
		...
		if (cryptoCoin != null) {
			conditions.add("((c.cryptoCoin like :cryptoCoin))");
		}
		if (margin != null) {
			conditions.add("((c.margin like :margin))");
		}
		if (paymentWindow != null) {
			conditions.add("((c.paymentWindow like :paymentWindow))");
		}
		if (fromDate != null) {
			LOGGER.debug("filtering with from date {}", new Date());
		...
		if (cryptoCoin != null) {
			createQuery.setParameter("cryptoCoin", cryptoCoin);
		}
		if (margin != null) {
			createQuery.setParameter("margin", margin);
		}
		if (paymentWindow != null) {
			createQuery.setParameter("paymentWindow", paymentWindow);
		}
		if (fromDate != null)
			createQuery.setParameter("fromDate", new Date(fromDate));
		if (toDate != null)
	...
	public Response<Object> getTradingSearch(Long userId, String search, TradeStatus tradeStatus, String country,
			...
			Integer page, Integer pageSize, Long userId2) {
		StringBuilder query = new StringBuilder(
				"select c.tradingId,c.tradeId, c.buyer, c.seller, c.tradeStatus, c.type,c.paymentType, c.creationTime,c.tradeAmount,c.totalBTC,c.tradeFee,c.fkUserId,c.partnerId,c.disputeId,c.disputeStatus,c.disputer,c.disputeDate,c.fiatCoin,c.appliedValue from Trading c");
		List<String> conditions = new ArrayList<>();
		...
		List<SearchTradeDto> response = list.parallelStream().map(o -> {
			...
			dto.setAppliedValue((BigDecimal) o[18]);
			return dto;
		}).collect(Collectors.toList());
	...
	public Response<Map<String, Object>> getPostTradeListForApp(OrderType orderType, Long userId) {
		...
		if (orderType != null) {
			getBuyOrderList = createAdvertisementDao.findByOrderTypeAndIsDeletedAndExchangeStatusTypeAndFkUserId(
					orderType, false, ExchangeStatusType.ENABLED, userId);
			getTotalCount = createAdvertisementDao.countByOrderTypeAndIsDeletedAndExchangeStatusTypeAndFkUserId(
					orderType, false, ExchangeStatusType.ENABLED, userId);
		}

		else {
			getBuyOrderList = createAdvertisementDao.findByIsDeletedAndExchangeStatusTypeAndFkUserId(false,
					ExchangeStatusType.ENABLED, userId);
			getTotalCount = createAdvertisementDao.countByIsDeletedAndExchangeStatusTypeAndFkUserId(false,
					ExchangeStatusType.ENABLED, userId);
		}
	...
	@Override
	public Response<Object> setEnableDisableParticular(Long userId, Long peerToPeerExchangeId,
			ExchangeStatusType exchangeStatusType, OrderType orderType) {
		if (peerToPeerExchangeId != null) {
			Optional<P2PAdvertisement> isDataExist = createAdvertisementDao
					.findByPeerToPeerExchangeIdAndFkUserId(peerToPeerExchangeId, userId);
			if (isDataExist.isPresent()) {
				Optional<P2PAdvertisement> list = createAdvertisementDao
						.findByExchangeStatusTypeAndOrderTypeAndPeerToPeerExchangeIdAndFkUserId(
								ExchangeStatusType.ENABLED, OrderType.BUY, peerToPeerExchangeId, userId);
				if (list.isPresent()) {
					list.get().setExchangeStatusType(ExchangeStatusType.DISABLED);
					createAdvertisementDao.save(list.get());
					return new Response<>(200, "Status Disabled Successfully");
				}
				Optional<P2PAdvertisement> list1 = createAdvertisementDao
						.findByExchangeStatusTypeAndOrderTypeAndPeerToPeerExchangeIdAndFkUserId(
								ExchangeStatusType.DISABLED, OrderType.BUY, peerToPeerExchangeId, userId);
				if (list1.isPresent()) {
					list1.get().setExchangeStatusType(ExchangeStatusType.ENABLED);
					createAdvertisementDao.save(list1.get());
					return new Response<>(200, "Status enabled Successfully");
				}
				Optional<P2PAdvertisement> list2 = createAdvertisementDao
						.findByExchangeStatusTypeAndOrderTypeAndPeerToPeerExchangeIdAndFkUserId(
								ExchangeStatusType.ENABLED, OrderType.SELL, peerToPeerExchangeId, userId);
				if (list2.isPresent()) {
					list2.get().setExchangeStatusType(ExchangeStatusType.DISABLED);
					createAdvertisementDao.save(list2.get());
					return new Response<>(200, "Status Disabled Successfully");
				}
				Optional<P2PAdvertisement> list3 = createAdvertisementDao
						.findByExchangeStatusTypeAndOrderTypeAndPeerToPeerExchangeIdAndFkUserId(
								ExchangeStatusType.DISABLED, OrderType.SELL, peerToPeerExchangeId, userId);
				if (list3.isPresent()) {
					list3.get().setExchangeStatusType(ExchangeStatusType.ENABLED);
					createAdvertisementDao.save(list3.get());
					return new Response<>(200, "Status Disabled Successfully");
				}
			}
		}
	...
	public Response<Object> deleteAdvertisement(Long userId, Long peerToPeerExchangeId) {
		if (peerToPeerExchangeId != null) {
			Optional<P2PAdvertisement> isDataExists = createAdvertisementDao
					.findByPeerToPeerExchangeIdAndFkUserId(peerToPeerExchangeId, userId);
			if (isDataExists.isPresent()) {
				createAdvertisementDao.deleteById(peerToPeerExchangeId);
				return new Response<>(200, "Advertisement Deleted Successfully");
			}
			return new Response<>(205, "Advertisement Not Present");
		}
		return new Response<>(205, "Advertisement Not Present For Following UserId");
	}

	@Override
	public Response<Object> addPaymentMethod(Long userId, PaymentMethodDto paymentMethodDto, String paymentType) {
		PaymentMethod paymentMethod = new PaymentMethod();

		paymentMethod.setAccountHolderName(paymentMethodDto.getAccountHolderName());
		paymentMethod.setAccountNo(paymentMethodDto.getAccountNo());
		paymentMethod.setIfscCode(paymentMethodDto.getIfscCode());
		paymentMethod.setImageUrl(paymentMethodDto.getImageUrl());
		paymentMethod.setUpiId(paymentMethodDto.getUpiId());
		paymentMethod.setUserId(userId);
		paymentMethod.setMobile(paymentMethodDto.getMobile());
		paymentMethod.setName(paymentMethodDto.getName());
		paymentMethod.setPaymentType(paymentType);
		paymentMethod.setAccountType(paymentMethodDto.getAccountType());
		paymentMethod.setBankName(paymentMethodDto.getBankName());
		paymentMethod.setBranch(paymentMethodDto.getBranch());
		paymentMethod.setRemark(paymentMethodDto.getRemark());
		paymentMethodDao.save(paymentMethod);
		return new Response<>(200, "payment methods added successfully");
	}

	@Override
	public Response<Object> editPaymentMethod(Long userId, PaymentMethodUpdateDto paymentMethodUpdateDto,
			String paymentType) {
		Optional<PaymentMethod> exists = paymentMethodDao.findByUserIdAndPaymentType(userId, paymentType);

		if (exists.isPresent()) {
			exists.get().setAccountHolderName(paymentMethodUpdateDto.getAccountHolderName());
			exists.get().setAccountNo(paymentMethodUpdateDto.getAccountNo());
			exists.get().setIfscCode(paymentMethodUpdateDto.getIfscCode());
			exists.get().setImageUrl(paymentMethodUpdateDto.getImageUrl());
			exists.get().setUpiId(paymentMethodUpdateDto.getUpiId());
			exists.get().setMobile(paymentMethodUpdateDto.getMobile());
			exists.get().setName(paymentMethodUpdateDto.getName());
			exists.get().setAccountType(paymentMethodUpdateDto.getAccountType());
			exists.get().setRemark(paymentMethodUpdateDto.getRemark());
			exists.get().setBankName(paymentMethodUpdateDto.getBankName());
			exists.get().setBranch(paymentMethodUpdateDto.getBranch());
			paymentMethodDao.save(exists.get());
			return new Response<>(200, "payment methods updated successfully");
		}
		return new Response<>(205, "payment method not found");
	}

	@Override
	public Response<Object> deletePaymentMethod(Long userId, Long paymentDetailId) {
		Optional<PaymentMethod> exists = paymentMethodDao.findByUserIdAndPaymentDetailId(userId, paymentDetailId);
		if (exists.isPresent()) {
			paymentMethodDao.deleteById(paymentDetailId);
			return new Response<>(200, "payment methods deleted successfully");
		}
		return new Response<>(205, "payment method not found");
	}

	@Override
	public Response<Object> getpaymentList(Long userId) {
		List<PaymentMethod> list = paymentMethodDao.findByUserId(userId);
		if (!list.isEmpty()) {
			return new Response<>(200, "Payment list fetched successfully", list);
		}
		return new Response<>(205, "payment method data not found");
	}

	@Override
	public Response<Object> getpaymentdetails(Long userId, Long paymentDetailId) {
		Optional<PaymentMethod> exists = paymentMethodDao.findByUserIdAndPaymentDetailId(userId, paymentDetailId);
		if (exists.isPresent()) {
			return new Response<>(200, "payment methods fetched successfully", exists.get());
		}
		return new Response<>(205, "payment method not found");
	}

	@Override
	public Response<Object> getuserdetails(Long userId) {

		Response<UserEmailAndNameDto> data = userClient.getEmailAndName(userId);
		List<P2PAdvertisement> dataExists9 = createAdvertisementDao.findP2PAdvertisementByFkUserId(userId);
		List<P2PAdvertisement> dataExists = createAdvertisementDao.findByFkUserIdAndExchangeStatusTypeAndStatusType(
				userId, ExchangeStatusType.DISABLED, StatusType.CLOSED);
		List<P2PAdvertisement> list = createAdvertisementDao.findFirst30ByOrderByCreationTimeDesc();
		List<P2PAdvertisement> dataExists1 = createAdvertisementDao
				.findByFkUserIdAndExchangeStatusTypeAndStatusTypeAndOrderType(userId, ExchangeStatusType.DISABLED,
						StatusType.CLOSED, OrderType.BUY);
		List<P2PAdvertisement> dataExists3 = createAdvertisementDao
				.findByFkUserIdAndExchangeStatusTypeAndStatusTypeAndOrderType(userId, ExchangeStatusType.DISABLED,
						StatusType.CLOSED, OrderType.SELL);
		Long dateDiff = ChronoUnit.DAYS.between(dataExists9.get(0).getCreationTime().toInstant(),
				new Date().toInstant());
		Map<String, Object> map = new HashMap<>();
		if (data.getStatus() == 200) {
			map.put("UserName", data.getData().getName());
			map.put("Kyc", data.getData().getKyc());
			map.put("joinindDate", data.getData().getJoinigDate());
			map.put("joinDays", data.getData().getJoinDays());
			map.put("CompletionRate", ((dataExists.size() / 30) * 100));
			map.put("last30daysTrade", list.size());
			map.put("firstTrade", dateDiff);
			map.put("buyTrades", dataExists1.size());
			map.put("sellTrades", dataExists3.size());
			Long size = Long.valueOf(dataExists.size());
			map.put("allTrades", size);
			return new Response<>(200, "User data fetched", map);
		}
		return new Response<>(205, "no data found");
	}

	@Override
	public Response<Object> getBankDetails(Long userId, Long p2pId) {
		List<PaymentMethodNewDto> finalList = new ArrayList<>();
		List<PaymentTypeMethod> list2 = paymentDao.findByp2pAdvertisementPeerToPeerExchangeId(p2pId);

		Stack<PaymentMethodNewDto> listNew = new Stack<>();

		if (!list2.isEmpty()) {
			for (PaymentTypeMethod dataList : list2) {
				Optional<PaymentMethod> dataFetched = paymentMethodDao.findByUserIdAndPaymentType(userId,
						dataList.getTypeName());
				PaymentMethodNewDto paymentMethodNewDto = new PaymentMethodNewDto();
				if (dataFetched.isPresent()) {

					paymentMethodNewDto.setAccountHolderName(dataFetched.get().getAccountHolderName());
					paymentMethodNewDto.setAccountNo(dataFetched.get().getAccountNo());
					paymentMethodNewDto.setIfscCode(dataFetched.get().getIfscCode());
					paymentMethodNewDto.setUpiId(dataFetched.get().getUpiId());
					paymentMethodNewDto.setImageUrl(dataFetched.get().getImageUrl());
					paymentMethodNewDto.setMobile(dataFetched.get().getMobile());
					paymentMethodNewDto.setName(dataFetched.get().getName());
					paymentMethodNewDto.setBankName(dataFetched.get().getBankName());
					paymentMethodNewDto.setAccountType(dataFetched.get().getAccountType());
					paymentMethodNewDto.setBranch(dataFetched.get().getBranch());
					paymentMethodNewDto.setRemark(dataFetched.get().getRemark());
					paymentMethodNewDto.setPaymentType(dataFetched.get().getPaymentType());

				}
				finalList.add(paymentMethodNewDto);
			}

			return new Response<>(200, "data Fetched", finalList);
		}
		return new Response<>(205, "data not Fetched");
	}

	@Override
	public Response<Object> getp2pDetails(Long p2pId) {
		Map<String, Object> map = new HashMap<>();
		Optional<P2PAdvertisement> data = createAdvertisementDao.findP2PAdvertisementByPeerToPeerExchangeId(p2pId);
		List<Trading> tradingList = tradingDao.findTradingBypeerToPeerExchangeId(p2pId);
		List<Trading> tradingList4 = tradingDao.findBypeerToPeerExchangeIdAndTradeStatus(p2pId, TradeStatus.PENDING);
		List<Trading> tradingList1 = tradingDao.findBypeerToPeerExchangeIdAndTradeStatus(p2pId, TradeStatus.COMPLETE);
		BigDecimal totalAmount1 = BigDecimal.ZERO;
		for (Trading dataGet1 : tradingList1) {
			totalAmount1 = totalAmount1.add(dataGet1.getTradeAmount());
		}
		List<Trading> tradingList2 = tradingDao.findBypeerToPeerExchangeIdAndTradeStatus(p2pId, TradeStatus.CANCEL);
		BigDecimal subtractAmount = data.get().getFirstTimePrice().subtract(totalAmount1);
		map.put("totalTrades", tradingList.size());
		map.put("openTrades", tradingList4.size());
		map.put("cancelTrades", tradingList2.size());
		map.put("completedTrades", tradingList1.size());
		map.put("amount", data.get().getFirstTimePrice());
		map.put("remainingAmount", subtractAmount);
		map.put("releaseAmount", totalAmount1);
		return new Response<>(200, "data Fetched", map);
	}

	@Override
	public Response<Object> gettradedetail(Long p2pId, Integer page, Integer pageSize) {
		Map<String, Object> map = new HashMap<>();
		List<Trading> tradingList = tradingDao.findTradingBypeerToPeerExchangeId(p2pId,
				PageRequest.of(page, pageSize, Direction.DESC, "tradingId"));
		if (!tradingList.isEmpty()) {
			map.put("List", tradingList);
			map.put("count", tradingList.size());
	...
	@Override
	public Response<Map<String, Object>> getBuySEllAdvertisementDetailsNew(OrderType orderType, Long userId,
	...
	@Override
	public Response<Object> cancelDispute(Long userId, String tradeId) {
		Optional<Trading> tradingData = tradingDao.findByTradeIdAndFkUserId(tradeId, userId);
		if (tradingData.isPresent()) {
			Optional<Escrow> escrowData = escrowDao.findByTradeId(tradeId);
			if (escrowData.isPresent()) {
				tradingData.get().setDisputeId(null);
				tradingData.get().setDisputeStatus(null);
				tradingData.get().setTradeStatus(TradeStatus.PENDING);
				escrowDao.deleteById(escrowData.get().getEscrowId());
				tradingDao.save(tradingData.get());
				return new Response<>(200, "dispute canceled with tradeId " + tradeId + " successfully");
			}
		}
		return new Response<>(205, "trade with tradeId " + tradeId + " not present");
	}
