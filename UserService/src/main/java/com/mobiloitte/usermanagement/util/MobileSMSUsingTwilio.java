...
public class MobileSMSUsingTwilio {
	...
	private static final int MAX_VERIFICATION_CODE_FOR_mobile = 100000;
	private static final int MIN_VERIFICATION_CODE_FOR_mobile = 999999;
	...
	// Twilio Send msg
	public Message sendCode(String sendTo, String bodyToSend, Integer otp) {
		final PhoneNumber to = new PhoneNumber(sendTo);
		final PhoneNumber from = new PhoneNumber(fromMobileNumber);
		final String body = bodyToSend + " " + otp;
		return new MessageCreator(to, from, body).create(twilioRestClient(accountSid, authToken));
	}
	
	public static Integer generateVerificationCodeMobileApp() {
		return new Random().nextInt(MIN_VERIFICATION_CODE_FOR_mobile - MAX_VERIFICATION_CODE_FOR_mobile + 1)
				+ MAX_VERIFICATION_CODE_FOR_mobile;
	}
