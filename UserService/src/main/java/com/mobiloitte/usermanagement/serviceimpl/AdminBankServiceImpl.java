...
import com.mobiloitte.usermanagement.dto.SearchBankDetailDto;
...
public class AdminBankServiceImpl implements AdminBankService {
	...
	public Response<Object> searchAndFilterBankAccountList(Long userId, SearchAndFilterBankDto searchAndFilterBankDto) {
		...
		try {
			StringBuilder query = new StringBuilder(
					"select bankDetailId, accountHolderName, accountNo, bankName, creationTime, updationTime, bankStatus, user.userDetail.firstName as firstName, user.userDetail.lastName as lastName, ifsc from BankDetails where isDeleted=false");
			...
			List<SearchBankDetailDto> response = list.parallelStream().map(o -> {
				...
				dto.setFirstName((String) o[7]);
				dto.setLastName((String) o[8]);
				dto.setIfsc((String) o[9]);
				return dto;
	...
	@Override
	public Response<Object> getView(Long userId, String bankName) {
		Optional<BankDetails> userData = bankDao.findByBankNameAndUserUserId(bankName, userId);
